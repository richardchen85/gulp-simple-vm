...
function optimizeTasks() {
    ...
    gulp.task('imagemin', function() {
        return optimizeImage(cfg.BASE_DIR + '/**', cfg.BUILD_DIR);
    });
    ...
    gulp.task('requirejs', function() {
        ...
        rjs.optimize(build, function() {
            console.log('requirejs complete...');
            // 删除r.js生成的build.txt
...
function devTasks() {
    ...
    gulp.task('watch', function() {
        var lessDir = path.resolve(cfg.BASE_DIR + '/' + cfg.less.dir),
            requirejsDir = path.resolve(cfg.requirejs.appDir),
            vmDir = path.resolve(cfg.velocity.dir);

        gulp.watch(cfg.BASE_DIR + '/**', function(event) {
            var file = event.path,
                evType = event.type,
                related = file.replace(path.resolve(cfg.BASE_DIR), ''),
                extname = path.extname(file),
                out = cfg.BUILD_DIR + '/' + path.dirname(related);
            
            if(file.indexOf(lessDir) > -1) {
                gulp.start('less');
            } else if (file.indexOf(requirejsDir) > -1) {
                gulp.start('requirejs');
            } else if (file.indexOf(vmDir) > -1) {
                gulp.start('vm');
            } else if (isConcatFile(file)) {
                gulp.start('concat');
            } else if (evType == 'changed' || evType == 'added' || evType == 'renamed') {
                if (util.isImageFile(extname)) {
                    compressFile(file, out, 'image');
                } else if (extname == '.js') {
                    compressFile(file, out, 'js');
                } else if (extname == '.css') {
                    compressFile(file, out, 'css');
                } else if (extname == '.html') {
                    compressFile(file, out, 'html');
                } else {
                    compressFile(file, out);
                }
            } else if (evType == 'deleted') {
