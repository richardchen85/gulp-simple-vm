...
function devTasks() {
    ...
    gulp.task('watch', function() {
        var imageDir = cfg.BASE_DIR + '/' + cfg.imageDir,
            jsDir = cfg.BASE_DIR + '/' + cfg.jsDir,
            cssDir = cfg.BASE_DIR + '/' + cfg.cssDir,
            htmlDir = cfg.BASE_DIR + '/' + cfg.htmlDir;

        cfg.less && gulp.watch(cfg.BASE_DIR + '/' + cfg.less.dir + '/**/*.less', ['less']);
        cfg.requirejs && gulp.watch(cfg.BASE_DIR + '/' + cfg.requirejs.appDir + '/**/*.js', ['requirejs']);
        cfg.velocity && gulp.watch(cfg.velocity.dir + '/**', ['vm']);
        ...
        gulp.watch([
            ...
            var isImage = file.path.indexOf(path.resolve(imageDir)) > -1;
            var isJs = file.path.indexOf(path.resolve(jsDir)) > -1;
            var isCss = file.path.indexOf(path.resolve(cssDir)) > -1;
            var isHtml = file.path.indexOf(path.resolve(htmlDir)) > -1;
    ...
    gulp.task('deploy', function() {
        ...
        copyStatic([cfg.BUILD_DIR + '/**', '!' + cfg.BUILD_DIR + '/**/*.html'], target);
...
// copy static files
function copyStatic(src, target) {
    gulp.src(src)
