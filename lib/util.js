...
Util.walk = function(dir, exclude) {
    var regExp, fileList = [];
    regExp = !exclude ? null : (function() {
        var pattern = exclude.map(function(val) {
            return '(' + val + ')';
        }).join('|').replace(/\\/g, '\\\\');
        return new RegExp(pattern);
    })();
    fs.readdirSync(dir).forEach(function(file) {
        var tmpPath = dir + path.sep + file;
        if(regExp && regExp.test(tmpPath)) {
            return;
        }
...
Util.exists = fs.exists || path.exists;

Util.existsSync = fs.existsSync || path.existsSync;
